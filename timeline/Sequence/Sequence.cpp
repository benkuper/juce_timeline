...
void Sequence::removeTimespan(float start, float end)
{
	...
	actions.addArray(totalTime->setUndoableValue(totalTime->floatValue(), totalTime->floatValue() - (end - start), true));
...
{
	Array<UndoableAction*> actions;
	actions.addArray(totalTime->setUndoableValue(totalTime->floatValue(), totalTime->floatValue() + length, true));
